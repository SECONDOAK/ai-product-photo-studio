...
export const ResolutionSelector: React.FC<ResolutionSelectorProps> = ({ selectedResolution, onSelect }) => {
  return (
    <div className="mt-6">
      ...
      <div className="flex flex-wrap gap-3">
        {RESOLUTIONS.map((res) => {
            ...
            return (
              ...
              >
                <span className={`text-sm font-bold mb-1 ${isSelected ? 'text-black' : 'text-gray-300'}`}>{res}</span>
                <div className={`flex items-center space-x-1 px-2 py-0.5 rounded-full ${isSelected ? 'bg-gray-200' : 'bg-black/40'}`}>
                    <CoinIcon className={`w-3 h-3 ${isSelected ? 'text-yellow-600' : 'text-yellow-500'}`} />
                    <span className={`text-xs font-medium ${isSelected ? 'text-gray-800' : 'text-gray-500'}`}>{priceSek} kr</span>
                </div>
              </button>
            );
        })}
